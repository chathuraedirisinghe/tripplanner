...
import com.jlanka.evtripplanner.DataLoader;
import com.jlanka.evtripplanner.GeofenceMonitor;
import com.jlanka.evtripplanner.GoogleAnalyticsService;
import com.jlanka.evtripplanner.LocationMonitor;
import com.jlanka.evtripplanner.MQTT.MQTTHelper;
import com.jlanka.evtripplanner.MQTT.MQTTPublisher;
import com.jlanka.evtripplanner.MainActivity;
import com.jlanka.evtripplanner.Model.FoundSuggestion;
import com.jlanka.evtripplanner.Model.GPSPoint;
import com.jlanka.evtripplanner.Model.Route;
...
import com.jlanka.evtripplanner.UserActivity.LoginActivity;
...
//                            System.out.println("Credits : "+credit);
    ...
    private Bitmap getMarkerIcon(String type,boolean status){
        ...
        if (type.equals("AC") ){
            if (status)
                image=(resizeMapIcons("l2a",85,100));
            else
                image=(resizeMapIcons("l2b",85,100));
        }
        else {
            if (status)
                image=(resizeMapIcons("dcfa",85,100));
            else
                image=(resizeMapIcons("dcfb",85,100));
        }
        return image;
    }
